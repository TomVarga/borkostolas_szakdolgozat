...
Még mielõtt bele mélyülnénk a szolgáltatások specifikus kezelésébe, mindenképpen érdemes néhány szót ejteni az Android alkalmazás szálkezelésérõl. A fejlesztés során nagyon könnyû bele futni a $NetworkOnMainThreadException$ -be, mely akkor következik be, amikor az alkalmazás hálózati mûveletet próbál végrehajtani a fõ szálában. Annak ellenére, hogy ez csak az Android $Honeycomb$ verziója után lett bevezetve, érdemes, mégis érdemes ezt elkerülnünk, feltéve hogy az alkalmazásunk támogatja ezt az SDK verziót. Ezt könnyen elkerülhetjük, ha minden hálózati mûveletet egy új szálon végzünk el, erre látható példa a \ref{lst:new thread for network} kódrészletben is.
...
Ennek a megszorításnak a betartása könnyen vezet a másik hibához, mely abból fakad, hogy a felhasználói felület elemeit próbáljuk manipulálni egy olyan szálból ami nem az alkalmazás fõ szála. Ez pedig az activity-kben a  $runOnUiThread\left(\right)$ segítségével érhetõ el, mely a $Thread$ -nél látottakhoz hasonlóan egy $Runnable$ objektumot vár.

\section{RemoteDAO}
Az Android alkalmazás távoli adatbázis elérésért felelõs osztálya mely az elõzõ fejezetben látott $DAO$ interface metódusait implementálja. A $POST$ mûveletekért elvégzésért az $org.apache.http.client$ csomag felelõs.
...
Az elsõ lépés az, hogy indítunk egy új szálat, hiszen hálózati mûveleteket készülünk csinálni, melyet egybõl követhet is a hálózat meglétének ellenõrzése a már megszokott módon. Eddig nincs semmi újdonság.
...
\section{Eredmények}
Az algoritmusok eredményének megjelenítése is tekinthetõ egy fajta szolgáltatásnak, mely a felhasználó azonosítója alapján generálja a megjelenítendõ tartalmat az android alkalmazás számra. Nem szorosan tartozik ide, érdekes lehet talán az hogy az algoritmust választó legördülõ menü használata magával vonja a számolások indítását. Ez a \ref{lst:dropdown onSelectChange} kódrészletben látott kódnak köszönhetõ.
...
\paragraph{Teszteljárás} TP\_\arabic{TP}
\paragraph{Leírás} Regisztráció megfelelõ adatok megadásával.
\paragraph{Bemenet} $user\_name = "tom"$ $user\_password_new = "tom123"$ $user\_password\_repeat = "tom123"$ $user\_email = "tom@tvarga.hu"$ $captcha = korrekt$
\paragraph{Action} Az adatok beírása után a "Regisztráció" gombra kattintok.
\paragraph{Elvárt kimenet} Hibaüzenet nélkül tér vissza.

\subsubsection{ID: TC\_\arabic{TC}}\addtocounter{TC}{1}
\paragraph{Teszteljárás} TP\_\arabic{TP}
\paragraph{Leírás} Regisztráció nem megfelelõ adatok megadásával.
\paragraph{Bemenet} $user\_name = "t"$ $user\_password_new = "tom123"$ $user\_password\_repeat = "tom123"$ $user\_email = "tom@tvarga.hu"$ $captcha = korrekt$
\paragraph{Action} Az adatok beírása után a "Regisztráció" gombra kattintok.
\paragraph{Elvárt kimenet} Hibaüzenettel tér vissza.

\subsubsection{ID: TC\_\arabic{TC}}\addtocounter{TC}{1}
\paragraph{Teszteljárás} TP\_\arabic{TP}
\paragraph{Leírás} Regisztráció hiányos adat mezõkkel.
\paragraph{Bemenet} $user\_name = ""$ $user\_password_new = "tom123"$ $user\_password\_repeat = "tom123"$ $user\_email = "tom@tvarga.hu"$ $captcha = korrekt$
\paragraph{Action} Az adatok beírása után a "Regisztráció" gombra kattintok.
\paragraph{Elvárt kimenet} Hibaüzenettel tér vissza.

\subsubsection{ID: TC\_\arabic{TC}}\addtocounter{TC}{1}
\paragraph{Teszteljárás} TP\_\arabic{TP}
\paragraph{Leírás} Regisztráció hibás captcha megadásával.
\paragraph{Bemenet} $user\_name = "tom1"$ $user\_password_new = "tom123"$ $user\_password\_repeat = "tom123"$ $user\_email = "tom@tvarga.hu"$ $captcha = hibás$
\paragraph{Action} Az adatok beírása után a "Regisztráció" gombra kattintok.
\paragraph{Elvárt kimenet} Hibaüzenettel tér vissza.
...
\addtocontents{toc}{\protect\setcounter{tocdepth}{3}}
\begin{itemize}	
\item ID: TP\_\arabic{TP}\addtocounter{TP}{1}
\item Tesztesetek: 
\forloop{TPTCStart}{\value{TPTCStart}}{\value{TPTCStart} < \value{TPTCStop}}{
	TC\_\arabic{TPTCStart}, 
}
\item Leírás: Ez a teszteljárás a regisztráció tesztelésének lépéseit írja le.
\item Lépések:
\begin{enumerate}
...
\subsubsection{ID: TC\_\arabic{TC}}\addtocounter{TC}{1}
\paragraph{Teszteljárás} TP\_\arabic{TP}
\paragraph{Leírás} Belépés megfelelõ adatok megadásával.
\paragraph{Bemenet} $user\_name = "tom"$ $user\_password = "tom123"$
\paragraph{Action} Az adatok beírása után a "Bejelentkezés" gombra kattintok.
\paragraph{Elvárt kimenet} Hibaüzenet nélkül tér vissza.

\subsubsection{ID: TC\_\arabic{TC}}\addtocounter{TC}{1}
\paragraph{Teszteljárás} TP\_\arabic{TP}
\paragraph{Leírás} Belépés nem megfelelõ adatok megadásával.
\paragraph{Bemenet} $user\_name = "t"$ $user\_password = "tom123"$
\paragraph{Action} Az adatok beírása után a "Bejelentkezés" gombra kattintok.
\paragraph{Elvárt kimenet} Hibaüzenettel tér vissza.
...
\begin{itemize}	
	...
	\item Leírás: Ez a teszteljárás a bejelentkezés tesztelésének lépéseit írja le.
	\item Lépések:
	\begin{enumerate}
