...
<select id='algoritmus' name='Algoritmus' onchange='onSelectChange()' style="width: 100%;">
	...
	Az Android alkalmazás megtervezése és implementálása nagyon jó alkalmat nyújtott a tanult $MVC$ tervezési minta alkalmazására, valamint magára a mobil fejlesztés menetének megismerésére. A web és mobil alkalmazás összekötése jó példa arra, hogy miképp lehet egy web alkalmazás szolgáltatását elérhetővé tenni egy natív mobil felületen. A fejlesztés során szerzett tapasztalat mindenképpen egy jó alapot adott a jövőbeli továbbfejlődéshez. Az alkalmazás jövőjét tekintve, a továbbfejlesztés egy lehetséges ága lehet az értékelés számítást teljes mértékben a mobil alkalmazásra mozgatni, majd ekkor a $CoHITS$ algoritmust egy a rendelkezésre álló részgráfon futtatni.
